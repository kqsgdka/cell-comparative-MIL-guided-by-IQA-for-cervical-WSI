...
#  子进程负责处理每张幻灯片（slide）的图块，使用 OpenSlide 和 DeepZoomGenerator 加载图像层级，并在每个图块上应用预处理（如阈值分割和连通域分析）。
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                ######
                # #被误删，后面需要重写写一下，通过阈值分割，然后选择面积大于200的连通域,这一版不是之前切割之后代码，所以效果不是很确定。还需要调整调整。
                gray_image = tile.convert("L")
                w, h = tile.size
                ...
                for region in measure.regionprops(labeled_image):
                    ...
                    # 符合条件的连通域
                    cell_count += 1
                if cell_count > 5 and w==self._tile_size and h==self._tile_size :
                    tile.save(outfile, quality=self._quality)
            except:
                pass
            self._queue.task_done()
...
# 通过命令行设置参数，（重叠区域，图块大小，放大倍率），然后提取土块并存储为指定格式文件
if __name__ == '__main__':
    Image.MAX_IMAGE_PIXELS = None
    parser = argparse.ArgumentParser(description='Patch extraction for WSI')
    parser.add_argument('-d', '--dataset', type=str, default='5classification', help='Dataset name')
    ...
    parser.add_argument('-v', '--slide_format', type=str, default='tiff', help='Image format for tiles [svs]')
    ...
    path_base = '/repository7503/kangll_data/WSI_data/5classification/ASC-H'
    if len(levels) == 2:
        out_base = os.path.join('./dataset', args.dataset, 'cell')
    else:
        out_base = os.path.join('./dataset', args.dataset, 'single')
    all_slides = glob.glob(path_base+'/*.tiff')
    # 将已经切片的跳过
    num = 0
    for slide in all_slides:
        slide_name  = slide.split(os.sep)[-1].split('.')[0]
        ttt = os.path.exists(os.path.join(out_base, 'kanglanlan', slide_name))
        if os.path.exists(os.path.join(out_base, 'kanglanlan', slide_name)):
            # print(os.path.join(out_base, 'kanglanlan', slide_name))
...
# python deepzoom_tiler_cell.py -m 0 2 -b 20 -d WSILabel -v tiff