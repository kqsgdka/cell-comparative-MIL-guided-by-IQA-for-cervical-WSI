...
#                                                                last_epoch=-1)
        ...
        for epoch_counter in range(self.config['epochs']):
            # 显示训练进度
            # with tqdm(train_loader, unit="batch", desc=f"Epoch {epoch_counter+1}/{self.config['epochs']}") as epoch_progress:
            for (xis, xjs) in tqdm(train_loader, desc=f"Epoch {epoch_counter + 1}/{self.config['epochs']}", leave=False):
                # print("开始训练~")
                optimizer.zero_grad()
